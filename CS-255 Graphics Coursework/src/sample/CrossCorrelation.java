...
/**
 * Contains the methods for cross correlation
 */
public class CrossCorrelation {
	...
	public static Image crossCorrel(Image image) {
		...
		for (int y = 0; y < height; y++) {
			for (int x = 0; x < width; x++) {
				// Normalise the pixel
				double redNormalised =
						((redArray[y][x] - redMin) * 255) / (redMax - redMin);
				double greenNormalised =
						((greenArray[y][x] - greenMin) * 255) / (greenMax - greenMin);
				double blueNormalised =
						((blueArray[y][x] - blueMin) * 255) / (blueMax - blueMin);

				// Round to 2 d.p.
				redNormalised =
						Math.round(redNormalised * 100.0) / 100.0;
				greenNormalised =
						Math.round(greenNormalised * 100.0) / 100.0;
				blueNormalised =
						Math.round(blueNormalised * 100.0) / 100.0;

				// Make new colour and set
				Color newColour =
						new Color(redNormalised / 255.0,
